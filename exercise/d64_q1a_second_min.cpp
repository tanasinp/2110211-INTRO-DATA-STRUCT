...
//  set<int> setto;
 while (!stackko.empty()){
    vetto.push_back(stackko.top());
    // setto.insert(stackko.top());
    stackko.pop();
 }
 sort(vetto.begin(),vetto.end());
// auto it = setto.begin();
// return *(setto.upper_bound(*it));
return *(upper_bound(vetto.begin(),vetto.end(),vetto[0]));
}
int main()
{
